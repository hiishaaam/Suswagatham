      ...
      <div className="flex-1 w-full relative -z-0 overflow-hidden">
         ...
         <div className="absolute bottom-[25%] left-0 w-full text-center z-10">
            ...
            <h1 className="font-display font-bold text-[48px] text-[#FAF7F0] leading-none drop-shadow-lg relative inline-block">
              <motion.span initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.5 }}>{data.coupleNames.split(' ')[0]}</motion.span> 
              <br/> 
              <motion.span animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 2, repeat: Infinity }} className="text-[#C9A84C] text-[32px] italic inline-block">&amp;</motion.span> 
              <br/>
              <motion.span initial={{ y: 20, opacity: 0 }} whileInView={{ y: 0, opacity: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>{(data.coupleNames.split(' & ')[1] || data.coupleNames.split(' AND ')[1] || '').split(' ')[0]}</motion.span>
            </h1>
